import java.io.File;
  ...
  public class Main {
      ...
      // Register an exit callback so a background job is marked Done the moment the OS
      // reports its exit, rather than relying on isAlive() polling timing.
      private static void trackBackground(Job job) {
          jobsList.add(job);
          if (job.process != null) {
              job.process.onExit().thenRun(() -> {
                          ...
                          if (execPath != null) {
                              try {
                                  ...
                                  if (parsed.runInBackground) {
                                      ...
                                      trackBackground(new Job(jobNum, p.pid(), parsed.commandStr, "Running", p));
                                  } else {
                                      p.waitFor();
                                  }
                              } catch (Exception e) {
                      ...
                      if (allFound) {
                          try {
                              ...
                              if (parsed.runInBackground) {
                                  ...
                                  trackBackground(new Job(jobNum, lastProcess.pid(), parsed.commandStr, "Running", lastProcess));
                              } else {
                      ...
                      if (allFound) {
                          try {
                              ...
                              if (parsed.runInBackground) {
                                  ...
                                  trackBackground(new Job(jobNum, pid, parsed.commandStr, "Running", lastProcess));
                              } else {
                                  for (Process p : processes) {
                                      if (p != null) {
                                          p.waitFor();
